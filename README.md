...
面向团队数据服务的 Rust HTTP/JSON 程序。提供内存数据集的批量写入、版本快照保存与历史快照读取能力，并在每次保存版本时把状态原子落盘，进程重启后可恢复。
...
`VDE_DATA_DIR` 指定持久化根目录，必填：未设置或为空时服务打印一行说明并以非零码退出；指向普通文件等非目录路径时同样报错退出，不会改动该文件。目录不存在时自动创建。`VDE_BIND` 可指定监听地址，默认 `127.0.0.1:8080`。按 Ctrl-C 停止服务。

## 持久化与恢复

- 每次 `POST /versions` 成功提交后，版本记录、快照内容与当时的数据集一起原子写入 `VDE_DATA_DIR` 下的 `v<N>.json`（先写临时文件再 rename，并 fsync 文件与目录）。提交前崩溃或写盘失败不会留下可见的这次保存，下次保存复用同一版本号。
- 写盘失败时该次保存整体失败：返回 `500` 与含 `error` 字段的 JSON 对象，数据集保持保存前的内容，版本号不递增。
- 启动时加载目录中的持久化状态：`POST /versions` 从已保存版本号加 1 继续分配，`/snapshots` 与 `/snapshots/diff` 可读回重启前的快照。目录中没有持久化文件时按全新实例启动，版本号从 1 开始。
- 恢复时逐条校验落盘条目（CRC32）：只写一半或校验不通过的条目按不存在处理并跳过，其后完整且校验通过的条目照常恢复，版本号按持久化顺序严格递增；无法确定哪些条目完整时以最后一个完整条目为准，其后的数据一律丢弃。
...
把数据集当前内容固化为不可变快照。版本号从 1 开始，每次成功保存严格递增 1；之后继续写入数据集不会改变任何已保存快照。保存空数据集也合法，得到记录数为 0 的空快照。返回新版本号 `version` 与该快照的记录总数 `total`。落盘失败时返回 `500` 与含 `error` 字段的 JSON 对象，数据集与版本号均不变。
